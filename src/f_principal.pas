...
begin
  if Application.MessageBox('O sistema será fechado.', 'Aviso', Mb_IconInformation + Mb_OkCancel) = IdOk then
  begin
...
begin
  ...
  begin
    Application.MessageBox('O CEP não foi informado', 'Aviso', Mb_IconInformation + Mb_Ok);
    edt_pesquisa.SetFocus;
    Exit;
  end;
  ...
  begin
    Application.MessageBox('O CEP informado está fora do formato esperado!' +#10+
  ...
  begin
    if RESTRequest1.Response.Content.IndexOf('erro') > 0 then
       Application.MessageBox('Cep não encontrado!' +#10+
